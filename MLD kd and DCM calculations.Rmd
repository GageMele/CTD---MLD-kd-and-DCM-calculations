...
ggplot()+
  geom_tile(data = CTD_CR, mapping= aes(x=date,y=depth, color = fluorescence), size = 1)+
...
CR_temp<-ggplot()+
  geom_tile(data = CTD_CR, mapping= aes(x=date,y=depth, color = temperature), size = 1)+
  geom_tile(data = mld_t_cr, mapping= aes(x=date,y=depth), color = "red", size = 1)+
  coord_fixed()+
  scale_y_reverse()+
  scale_color_continuous(type = "viridis")+
  ggtitle("temperature CR & MLD from temp")+
scale_x_continuous(breaks = pretty(CTD_CR$date, n = 10))+
  theme_cowplot()
...
CR_den<-ggplot()+
  geom_tile(data = CTD_CR, mapping= aes(x=date,y=depth, color = density), size = 1)+ #using calculated density instead of sigmaTheta
